...
export async function POST(req: Request) {
  ...
  const clean = (({ address, chainId, name, symbol, image, description, creator, split, pool, tokenId, lockPct, lockDays, devBuyEth, socials, gameName, txHash, referrer, createdAt, version, poolId, creatorFeePips, creatorShareBps, hook }) =>
    ({ address, chainId, name, symbol, image, description, creator, split, pool, tokenId, lockPct, lockDays, devBuyEth, socials, gameName, txHash, referrer, createdAt, version, poolId, creatorFeePips, creatorShareBps, hook }))(token);
  const client = db();
  if (client) {
