...
contract RewardsDistributor is Ownable {
    ...
    event EpochSwept(uint256 indexed epochId, address to, uint256 amount);
    ...
    function publish(address rewardToken, bytes32 root, uint256 total, string calldata dataURI) external onlyOwner returns (uint256 id) {
        require(root != bytes32(0) && total > 0 && rewardToken != address(0), "bad epoch");
        id = ++epochCount;
    ...
    function claim(uint256 epochId, address account, uint256 amount, bytes32[] calldata proof) external {
        ...
        // An epoch can never pay out more than it was funded with, so it can never reach another epoch's balance.
        require(e.claimed + amount <= e.total, "epoch exhausted");
    ...
    function sweep(uint256 epochId, address to) external onlyOwner {
        ...
        // Clearing the root closes the epoch: a straggler can no longer claim it out of a later epoch's funds.
        e.root = bytes32(0);
        e.claimed = e.total;
        if (left > 0) IERC20(e.rewardToken).safeTransfer(to, left);
        emit EpochSwept(epochId, to, left);
    }
}
